...
"""
    ...
    When the 1st byte is 6 or 8, the second byte doesn't matter.
    +----------+--------------------------+
    ...
    +----------+-----------------+-----------------+--------------+
    | 2nd Byte | 1st byte is 0-3 | 1st byte is 4-5 |1st byte is 9 | 
    +----------+-----------------+-----------------+--------------+
    |        0 | Stop motor      | No change       | Centre servo |
    |        1 | Speed = slow    | Increase angle  | Servo up     |
    |        2 | Speed = fast    | Decrease angle  | Servo down   |
    +----------+-----------------+-----------------+--------------+

    The second byte ranges from 0-4 when byte 1 is 7 as below.
    +----------+-----------------+
    | 2nd Byte | 1st byte is 7   |
    +----------+-----------------+
    |        0 | Stop motor      |
    |        1 | Forward         |
    |        2 | Backward        |
...
'''
   Function:  stick_center_snap
   ------------------------------
   Attempt to compensate for calibration and loose stick.
   value:  		value to compare snap against
   snap: 		threshold to differentiate a command from a snapback signal
   returns:   	int
'''
def stick_center_snap(value, snap=0.2):
    ...
    if value >= snap or value <= -snap:
        ...
        return 0.0																# Probably loose joystick reading
...
'''
   ...
   x:  			x-axis joystick position
   y: 			y-axis joystick position
   returns:   	null
'''
def calcTrackSpeed(x,y):
    ...
    updateCount = updateCount + 1												# Used to keep track of when the message should be resent (allowing for dropping of packets)
    ...
    currentCommand = "{}{}".format(motorDir,spd)								# Format the current command as a 2 byte string to be sent

    if (lastCommand == currentCommand and updateCount > 50):					# Packet was probably dropped - Stop the motors
        sendCommand("00")
        lastCommand = "00"
        updateCount = 0
    elif (lastCommand != currentCommand):										# New command, sent immediately
        sendCommand(currentCommand)
        lastCommand = currentCommand
        updateCount = 0
    

''' Function: baseServoMove
	-----------------------
	This function takes the left joysticks y value and send the value for the servo position.
	y:			Y value of the left joystick
	returns:	null
'''
def baseServoMove(y):
    yPos = 0;

    global lastCommandBase
    global updateCountBase

    if (y < 0.2 and y > -0.2):
	    yPos = 2																# Default position
    elif (y >= 0.2 and y < 0.75):
	    yPos = 1
    elif (y >= 0.75):
	    yPos = 0
    elif (y <= -0.2 and y > -0.75):
    ...
    updateCountBase = updateCountBase + 1	

    currentCommand = "9{}".format(yPos)											# Format the current command as a 2 byte string to be sent

    if (lastCommandBase == currentCommand and updateCountBase > 100):					# Packet was probably dropped - Set to default position
        sendCommand("92")
        lastCommandBase = "92"
        updateCountBase = 0
    elif (lastCommandBase != currentCommand):										# New command, sent immediately
        sendCommand(currentCommand)
        lastCommandBase = currentCommand
        updateCountBase = 0
