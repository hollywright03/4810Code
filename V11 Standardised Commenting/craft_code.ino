...
void setup() {
  ...
  servoBase.attach(3);  //D9
  ///servoBase.attach(12);
  servoLeft.write(LCLOSE);
  servoRight.write(RCLOSE);
  servoBase.write(BASECENTRE);
...
/*
 * Function: base_servo_move
 * -------------------------
 * Move the base servo between down position and up position, 
 * with intermediate positions as well.
 * 
 * pos:     Specifies the position of the servo, varying roughly
 *          linearly between 0 and 4, with 0 fully up, 4 fully 
 *          down and 2 centred.
 *        
 * returns: Nil.
 */
void base_servo_move(int pos) {
  int halfUp = (BASECENTRE - BASEUP)/2 + BASEUP;
  int halfDown = (BASEDOWN-BASECENTRE)/2 + BASECENTRE;
  switch (pos) {
    case 0:
      servoBase.write(BASEUP);
      break;
    case 1:
      servoBase.write(halfUp);
      break;
    case 2:
      servoBase.write(BASECENTRE);
      break;
    case 3:
      servoBase.write(halfDown);
      break;
    case 4:
      servoBase.write(BASEDOWN);
      break;
    default:
      servoBase.write(90);
      break;
