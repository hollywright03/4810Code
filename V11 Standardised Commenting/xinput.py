...
'''
   ...
   ------------------------------
   Types of inputs expected from the controller.
   ctypes.Structure:           a struct containing the data types expected for each controller parameter
'''
...
'''
   Class:  XINPUT_STATE
   ------------------------------
   States expected from the controller.
   ctypes.Structure:           a struct containing the data types expected for each status parameter
'''
...
'''
   ...
   ctypes.Structure:           a struct containing the data types expected for each vibration parameter
'''
...
'''
   Function:  struct_dict
   ------------------------------
   Take a ctypes.Structure and return its field/value pairs
    as a dict.
   struct:         The struct to turn into a dictionary
   returns:        Dictionary
'''
...
'''
   Function:  get_bit_values
   ------------------------------
   Get bit values as a list for a given number.
   number:         The number to turn into binary
   size:           Fixed at 32 bit number
   returns:        List
'''
...
'''
   ...
   ------------------------------
   Defines variables and functionality for game events
'''
class PygameEventDispatcher(object):
    ...
    # Define the on button press event
    def on_button(self, *args, **kwargs):
        state = args[0]
        try:
            value = args[1]
        except IndexError:                      # The args array passed in was not large enough
            value = kwargs.get('value', 0)
        print('on_button', args, kwargs)

        if state in self.hat_map:               # If the state found is part of the hat_map values
            # Convert the button to JOYHATMOTION for pygame
            etype = JOYHATMOTION
            if state in self.hat_y_ids:
                self.hat_y = value * self.hat_map[state]            # Convert to values recognised by the JOYHATMOTION
            elif state in self.hat_x_ids:
                self.hat_x = value * self.hat_map[state]
            # We need to assume hat=0
            pygame.event.post(pygame.event.Event(
                etype, joy=self.joystick.device_number, hat=0, value=(self.hat_x, self.hat_y)))
        else:
            # It's a regular button in pygame.
            if value == 0:                                          # Button up pressed
                etype = JOYBUTTONUP
            else:
                etype = JOYBUTTONDOWN                               # Button down pressed
            try:
                button = self.button_map[state]                     # Try return the corresponding button map value
    ...
    def on_axis(self, *args, **kwargs):
        ...
        try:
            ...
            pygame.event.post(pygame.event.Event(               # Send joystick event
                JOYAXISMOTION,
    ...
    def on_state_changed(self, *args, **kwargs):                # Do nothing
        pass

    def on_missed_packet(self, *args, **kwargs):                # Do nothing
        pass
...
'''
class XInputJoystick(object):  #event.EventDispatcher): 
    ...
    # Initialise the x input joystick values
    ...
    # Gets the state of the object that calls this function
    def get_state(self):
        ...
        if res == ERROR_SUCCESS:                                # Successfully retrieved state
            return state
        if res != ERROR_DEVICE_NOT_CONNECTED:                   # No device connected
            raise RuntimeError(
                "Unknown error %d attempting to get state of device %d" % (res, self.device_number))
        # else return None (device is not connected)

    # Returns a Boolean representing object connection status
    def is_connected(self):
        return self._last_state is not None

    # Returns all connected devices in a list
    @staticmethod
    ...
    # Sets the vibration speed within the joystick - Not needed 
    ...
    # Gets joystick events
    def dispatch_events(self):
        "The main event loop for a joystick"
        state = self.get_state()
        if not state:                                       # No joystick connected
            raise RuntimeError(
                "Joystick %d is not connected" % self.device_number)
        if state.packet_number != self._last_state.packet_number:       # If a new packet has been recieved
            # state has changed, handle the change
            self.update_packet_count(state)                 # Update packet cound
            self.handle_changed_state(state)                # Update state change
        self._last_state = state                            # Save new state

    # Increments based on packets recieved and packets dropped
    ...
    # Execute fucntions to deal with a change in joystick state
    ...
    # Call functions to deal with state change
    ...
    # Call fucntions to deal with state changes that aren't to do with buttons
    ...
    # Execute functionality to deal with button state changes - note, this is all events
    ...
    # Execute functionality to deal with button state changes 
...
# not used - Works out optimal sampling rate for the operation
...
'''
   Function:  sample_first_joystick
   ------------------------------
