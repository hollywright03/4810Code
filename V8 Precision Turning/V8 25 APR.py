...
"""
    ...
    +----------+--------------------------+
    |        0 | Left DC motor, forward   |
    |        1 | Left DC motor, backward  |
    |        2 | Right DC motor, forward  |
    |        3 | Right DC motor, backward |
    |        4 | Left Servo               |
    |        5 | Right Servo              |
    |        6 | LED TOGGLE               |
    +----------+--------------------------+
...
def calcTrackSpeed(x,y,lastLValue,lastRValue):
    #print "{}:{}".format(round(x,2),round(y,2))
    mag = math.sqrt(x*x+y*y)
    angle = math.atan2(y,x) * 180 /3.14159
    ...
    if(angle > 0 and angle < 90):
        leftMotor = 1
        rightMotor = angle/45 - 1
        
    elif(angle > 90 and angle < 180):
        leftMotor = 3 - angle/45
        rightMotor = 1
        
    elif(angle < 0 and angle > -90):
        leftMotor = 1 + angle/45
        rightMotor = -1
        
    elif(angle < -90 and angle > -180):
        leftMotor = -1
        rightMotor = -3 - angle/45

    leftMotor *= mag
    rightMotor *= mag
    #print "{}:{}".format(round(leftMotor,2),round(rightMotor,2))

    if(abs(leftMotor - lastLValue) > sendThresh):
        lastLValue = leftMotor

        if(abs(leftMotor) > 0.8):
            spd = 2
        elif (abs(leftMotor) > 0.2):
            spd = 1
        else:
            spd = 0
        ...
        sendCommand("{}{}".format(direction,spd))
        
        """if ((leftMotor - lastLValue) > sendThresh and (leftMotor - lastLValue) > 0):
            #LastLValue = leftMotor
            sendCommand("0{}".format(spd))
        elif (abs(leftMotor - lastLValue) > sendThresh and (leftMotor - lastLValue) < 0):
            #LastLValue = leftMotor
            sendCommand("1{}".format(spd))"""
        

    if(abs(rightMotor - lastRValue) > sendThresh):
        lastRValue = rightMotor
        #sendCommand("STPR{}".format(round(rightMotor,1)))

        if(abs(rightMotor) > 0.8):
            spd = 2
        elif (abs(rightMotor) > 0.2):
            spd = 1
        else:
            spd = 0
        ...
        """if ((rightMotor - lastRValue) > sendThresh and (rightMotor - lastRValue) > 0):
            LastRValue = rightMotor
            sendCommand("2{}".format(spd))
        elif (abs(rightMotor - lastRValue) > sendThresh and (rightMotor - lastRValue) < 0):
            LastRValue = rightMotor
            sendCommand("3{}".format(spd))"""

    return [lastLValue, lastRValue]
